...
- (void)initSubViews {
    ...
    _bottomLabelColor = UIColorFromRGB(0xA5A5A8);
    ...
    _mainLineColor = UIColorFromRGB(0x29282B);
    _viceLineColor = UIColorFromRGB(0xDFDFDF);
    ...
    for (int i = 0; i <= totalLines; i++) {
        ...
        if (i % numberOfScale == 0) {  // 长线
            ...
            if (self.showBottomLabel) {
                ...
                label.font = [UIFont systemFontOfSize:self.bottomLabelFontSize];
...
- (void)updateLines {
    ...
    for (int i = 0; i < self.lines.count; i++) {
        ...
        CGFloat distance = fabs(self.rulerContainer.contentOffset.x + self.width/2.0 - lineV.centerX);
        if (distance > 120) {
            lineV.alpha = 0;
        } else {
