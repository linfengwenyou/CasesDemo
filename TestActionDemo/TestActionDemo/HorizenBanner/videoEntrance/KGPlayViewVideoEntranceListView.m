...
- (void)setFromPoint:(CGPoint)fromPoint {
    ...
    self.layout.fromPoint = fromPoint;
    
    // 准备进行刷新, 预处理机制
    [self.layout prepareForShowType:KGVideoEntranceLayouType_hide];
    [self resetBackSortForCell];
    
}
#pragma mark - public
- (void)showContainer {
    self.collectionView.alpha = 1;  // 显示出来
    [self.layout refreshLayoutForShow:KGVideoEntranceLayouType_Show];
...
- (void)hideContainer {
    [self resetBackSortForCell];
    [self.layout refreshLayoutForShow:KGVideoEntranceLayouType_hide];
...
// 调整UI展示顺序
- (void)resetBackSortForCell {
    for (NSIndexPath *indexPath in self.layout.visibleSortedIndex) {
        @autoreleasepool {
            UICollectionViewCell *cell = [self.collectionView cellForItemAtIndexPath:indexPath];
            [self.collectionView sendSubviewToBack:cell];
        }
